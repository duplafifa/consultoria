  ...
  return (
    <div className="p-8 bg-[#0a0f12] text-white min-h-screen">
      <div className="max-w-4xl mx-auto">
        ...
        <nav className="flex items-center gap-1 p-1 bg-zinc-900 rounded-2xl mb-8 border border-zinc-800">
          <button 
            onClick={() => setActiveTab('courses')} 
            className={`flex-1 px-6 py-3 rounded-xl font-bold text-sm transition-all ${activeTab === 'courses' ? 'bg-emerald-600 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            Gerenciar Cursos
          </button>
          <button 
            onClick={() => setActiveTab('users')} 
            className={`flex-1 px-6 py-3 rounded-xl font-bold text-sm transition-all ${activeTab === 'users' ? 'bg-emerald-600 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            Aprovar Usuários
          </button>
        </nav>
